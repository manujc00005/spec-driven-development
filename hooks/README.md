...
| `graphify-stale-reminder` | `SessionStart` | Read-only — checks `GRAPH_REPORT.md` existence and mtime | Wired by default (safe) | Relying on a stale or missing architecture map for impact analysis. Warns if `GRAPH_REPORT.md` is absent or >7 days older than the newest source file. **Never blocks** — reminder only (exit 0, always safe). Graphify is optional; hook is safe with or without it. |

...
Thirteen hook families (26 scripts — all ship as a `.ps1`/`.sh` pair; `maven-compile` is deprecated and kept installable). Each hook is a small, single-purpose script invoked by Claude Code at a specific lifecycle point, configured in `.claude/settings.json` — start from [`settings.template.json`](../settings.template.json) (Windows/PowerShell) or [`settings.template.sh.json`](../settings.template.sh.json) (macOS/Linux/bash) at the repo root; both wire the same hook set. A third wiring, [`hooks.json`](hooks.json), is the plugin's: it carries the same default set with `${CLAUDE_PLUGIN_ROOT}` paths, and `scripts/check-consistency.sh` keeps it equivalent to the bash template (spec 044).
...
| `graphify-scan-reminder` | `PreToolUse` on `Grep`/`Glob` | Read-only except a throttle marker (`.graphify/.scan-nudge`) — never blocks the tool call | Wired by default in both templates | Forgetting the graph exists and paying for broad scans. When `.graphify/GRAPH_REPORT.md` (or legacy root report) exists, nudges the session to prefer graph-first context (`/graphify-context`, `graphify review-context`); at most one nudge per 30 min. `SDD_GRAPHIFY_NUDGE=0` disables. **Never blocks** — exit 0 always |
| `scope-keeper-reminder` | `PreToolUse` on `Edit`/`Write`/`NotebookEdit` | Read-only except a throttle marker in the system temp dir — never blocks the tool call | Wired by default in both templates | The mindset skills are declared "always in effect", but a skill is *model-invoked*: its rules only reach context if the assistant chooses to load it. `scope-keeper` names its own trigger — "before your first edit" — and this hook is the harness observing it. Emits the load-bearing scope rules **once per session** (keyed on `session_id`, falling back to a 1h throttle when that field is absent). `SDD_SCOPE_REMINDER=0` disables. **Never blocks** — exit 0 always |
...
- `graphify-scan-reminder` is a no-op (exit 0, no output) when no Graphify report exists, when `SDD_GRAPHIFY_NUDGE=0`, or when it already nudged within the last 30 minutes (mtime of `.graphify/.scan-nudge`). Otherwise it emits a single one-line reminder to use graph-first context before broad scans. It never inspects the Grep/Glob arguments and never blocks — targeted searches are legitimate; this is reinforcement of the graph-first doctrine (spec 010 D006, spec 011), not enforcement.
- `scope-keeper-reminder` fires once per session, before the first `Edit`/`Write`/`NotebookEdit`, and is silent for the rest of it — re-nagging on every edit is what gets a hook switched off. It is a no-op (exit 0, no output) when `SDD_SCOPE_REMINDER=0`, or when this session was already reminded. It never inspects what is being edited and **never blocks**: scope is a judgement, not a predicate, so a hook that blocked on it would fire on correct work (spec 036 D002). The message is a short excerpt; `skills/scope-keeper/SKILL.md` remains the source of truth, and `scripts/mindset-hook.test.sh` fails if the two drift apart.

...
| R7 | **`--notify` becomes a command-injection surface** carrying agent-authored text. | High | Executed without a shell, argument vector fixed, event delivered as JSON on stdin. Covered in T020's security review. |
...
| R9 | **Secrets land in `run.jsonl`** and the log becomes unshareable. | Medium | Redaction at the writer, not at call sites; AC-012's sentinel grep. |
| R10 | **The real E2E fails for reasons the stub cannot model** (SDK session semantics, prompt shape of `agents/*.md`). | Medium | T018 is scheduled early enough to still be a design signal; the Assumptions section already flags that a prompt-shape mismatch is a finding, not a licence to fork the agent files. |

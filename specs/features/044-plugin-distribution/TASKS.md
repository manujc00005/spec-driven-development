...
- [ ] T001 - Spike the two open assumptions: write minimal `.claude-plugin/plugin.json` (name `sdd`)
...
- [ ] T002 - Finalise `.claude-plugin/plugin.json`: `name` `sdd`, `version` `0.1.0`, `description`,
  ...
  prints 0.
- [ ] T003 - Finalise `.claude-plugin/marketplace.json`: marketplace `spec-driven-development`, owner,
  ...
  exits 0.
- [ ] T004 - Create `.codex-plugin/plugin.json` with the same identity, `"skills": "./skills/"`, and
  ...
  exits 0.
- [ ] T005 - Create `hooks/hooks.json` by transcribing the `hooks` object of
...
- [ ] T006 - Extend `scripts/check-consistency.sh`: call `check_settings_wiring("hooks/hooks.json")`
...
- [ ] T007 - Add two lines to the "Security checklist" list in `skills/security-review/SKILL.md`,
...
- [ ] T008 - Add one line to the "Review checklist" list in `skills/qa-review/SKILL.md`, after "Is
...
- [ ] T009 - Add an "Install as a plugin" section to `docs/INSTALL.md` immediately after the title
...
- [ ] T010 - In `README.md`, make the Quickstart (line ~234) and Installation (line ~571) sections
...
- [ ] T011 - Add two cases to `scripts/check-consistency.test.sh` after the FR-007 case: a fresh copy
...
- [ ] T012 - Re-run the Claude Code path against the final manifests: `claude plugin marketplace
  ...
  prints 0.
- [ ] T013 - Run the Codex path: `codex plugin marketplace add "$(pwd)"`, `codex plugin add
  ...
  AC-007.
- [ ] T014 - Drive a session in a disposable project: `d=$(mktemp -d) && cd "$d" && git init -q`
...
- [ ] T015 - Confirm the contract diff is empty and the gate is green on the final tree:

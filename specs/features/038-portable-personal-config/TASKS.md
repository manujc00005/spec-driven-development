...
- [ ] T001 - Define the shared manifest (`CLAUDE.md`, `settings.json`, `agents/`,
...
- [ ] T002 - Build the sandbox harness in `scripts/personal-config.test.sh`: synthetic `$HOME`,
...
- [ ] T003 - Credential detector: match `token`, `secret`, `api[-_]?key`, `password`, `Bearer `,
...
- [ ] T004 - `scripts/export-personal-config.sh`: copy the T001 manifest slice into
...
- [ ] T005 - Classifier — the core of FR-004: given payload file and target, return
...
- [ ] T006 - `MEMORY.md` additive merge: append only index lines absent from the target, under a
...
- [ ] T007 - `settings.json` merge: add only top-level keys absent in the target; a local key
...
- [ ] T008 - `scripts/import-personal-config.sh`: walk the payload, apply T005 (with T006/T007
...
- [ ] T009 - Wire the call site at the end of `install.sh` before `Done.`, add `--no-personal` and
...
- [ ] T010 - Port export and import to PowerShell with the same manifest, semantics and summary
...
- [ ] T011 - Fill the sandbox suite: import into empty, import into populated-with-conflicts,
...
- [ ] T012 - Unit cases for T003, T005, T006, T007 called directly, including the negatives (a
...
- [ ] T014 - `bash scripts/check-consistency.sh` and the full local gate green; add the new suite

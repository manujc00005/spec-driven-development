...
Add `scripts/check-consistency.sh` (bash + embedded python3, same pattern as `install.sh`), with optional `--fix` flag for auto-correcting safe violations (README count markers), a self-test script that injects each drift class into a temp copy of the repo, count markers in `README.md`, and a GitHub Actions workflow that runs the checker on push/PR to `main`.
...
5. **README counts (FR-008, FR-012):** parse `<!-- count:<key> -->N<!-- /count -->` spans; compare against computed values; enforce the minimum marker set (`skills-total`, `hook-families-total`, `templates-total`, `agents-total`, `<profile>-skills`, `<profile>-hooks`, `<profile>-templates` for profiles shown in the table). Add the markers to `README.md` as part of implementation. When invoked with `--fix` flag, auto-update markers to computed values and report `[FIXED]` per marker; non-auto-fixable violations (orphans, missing shipped, planned drift, hook pairs, wiring) still cause exit 1 and block README changes.
...
- `scripts/check-consistency.test.sh` covers AC-001..007, AC-009, and AC-010 by mutation-testing a temp copy of the repo (see approach step 8). It runs in the same CI workflow after the real check. The --fix cases are tested by invoking with `--fix` on mutation-injected repos, verifying that README markers are updated and exit code matches expectations.
- AC-008 verified by observing the first real workflow run (manual, one-time).
- Local manual run on Git Bash (Windows) before pushing, including `--fix` scenarios.

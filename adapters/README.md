...
| Lifecycle steps (create→plan→analyze→implement→review→close) | ✅ `skills/*/SKILL.md` slash commands | ⚠️ prompts in `codex/prompts/` (lifecycle spine only) | Codex prompts derived from the same skill procedures; curated spine, not all 65 skills. |
...
| Skill contracts (`## SDD Contract`) | ✅ on all 65 skills | ✅ reused as provider-neutral metadata | The `provider_specific` flag already anticipated this split. |
